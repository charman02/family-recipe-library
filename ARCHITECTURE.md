...
| `UserProfile.jsx` | `/u/:userId` | Another person's read-only profile — name, photo, their `profile_visibility`, the caller-side friend button (add / accept / requested / remove), and their recipe + post counts, each already gated by `can_view` / `can_view_post` server-side. Content grid via `ProfileContent`. Below the friend action sits a deliberately quiet **"Block {name}"** link (#85) — a safety control, not a social one, so nobody should reach it by accident; hidden on your own profile — whose two-tap confirm names every consequence (mutual invisibility, the deleted friendship, and that a recipe you already sent them stays theirs) rather than asking "are you sure?" about nothing. On success it navigates to `/friends`, because this page is now a 404 for the caller. |
...
- **Verifying changes:** backend has `pytest` (**433 tests** across `tests/`);
  frontend has Vitest + React Testing Library (**683 tests in 49 files**) — run
