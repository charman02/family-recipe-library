...
| `PostPage.jsx` | `/posts/:id` | A single shared meal, read-only (#71) — photo, dish name, description, author (tap → their profile), and "See the recipe →" when one's attached. Reached by opening a meal in Browse; a real permalink for any post the viewer may see. Read auth is the backend's `GET /posts/{id}` (`can_view_post`): a public post opens for anyone, anything else the viewer isn't entitled to reads as "not available" (404), never confirming it exists. No like button, no request action yet (Phase 2). |
| `AddChooser.jsx` | `/add` | The add chooser — two paths: **"Share a meal"** → `PostComposer` at `/add/meal`, and **"Keep a recipe"** → `PlantRecipe` at `/add/recipe`. |
...
| `PlantRecipe.jsx` | `/add/recipe` | Add-a-recipe flow that lands **directly on the say/paste screen** (`<PasteRecipe>`, heading "Add it your way" — speak/paste → LLM-structured draft); there is no doorway/chooser step. The plain `<RecipeForm>` is reached only via a demoted **"Rather type it in?"** link at the bottom of that screen. Both land on the same `RecipeForm`, which carries the optional "Passed down from" field and `<VisibilityChoice>` (only a dish name is required) → `<SaveCelebration>` (the timer→poof→card reveal, which is now the terminal "saved" screen: tap the card to view, or the button to hand it off) → HandoffInvite. |
...
- **Verifying changes:** backend has `pytest` (**320 tests** across `tests/`);
  frontend has Vitest + React Testing Library (**541 tests in 45 files**) — run
