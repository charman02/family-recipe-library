...
| `KitchenSections.jsx` | The Home "explore" sections, built from `lib/kitchenFacts.js`. The page was a hero plus repeated card grids; since a feed of other people's content is exactly what this product disclaims, the interest is drawn out of the thesis instead (who's in the kitchen, a quotable line of the day, unfinished recipes, a kitchen-at-a-glance) rather than borrowed from a social feed. |
...
| `Home.jsx` | `/` | A peach hero (with decorative dish discs) that opens the Kitchen, a coral "N recipes to cook" bar, and marker-titled sections of recipe cards (community "passed down lately" + "your kitchen"). Welcome empty state on first run. |
...
| `heroReason.js` | Picks *why* a given recipe is the one in the Home hero, so the biggest thing on the page carries a reason rather than reading as an arbitrary pick. |
| `kitchenFacts.js` | Derived views over the user's own recipes for `KitchenSections`, all computed client-side from data Home already fetches — no new endpoint, and no borrowed social feed. |
...
- **Verifying changes:** backend has `pytest` (**244 tests** across `tests/`);
  frontend has Vitest + React Testing Library (**512 tests in 38 files**) — run
