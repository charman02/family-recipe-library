...
│                           (also carries the OpenGraph/Twitter meta + favicon,
│                            so an invite link unfurls as a branded card;
...
| `RecipeForm.jsx` | Shared create/edit form body, reused by PlantRecipe and EditRecipe. Marker-titled sections with persistent `FieldLabel`s and paired name-vs-measurement / step-vs-step-note fields, plus an optional **"Passed down from"** name field that feeds `origin_attribution` (the paste parser seeds it when the model detects a source; the story prompt follows it). Handles photo upload incl. iPhone HEIC → JPEG conversion (lazy-loaded `heic2any`). Errors go through `toUserMessage`. |
| `PasteRecipe.jsx` | The **primary** add-a-recipe door: one big text box where you paste or dictate the whole recipe as free text, sent to `POST /recipes/parse` (LLM) and returned as a pre-filled `RecipeForm` draft to correct before saving. On no key / any failure the endpoint answers `ai: false` and this falls back to the local line-based parser (`lib/parseRecipeText.js`), so the door always works. Amounts are re-graded server-side, never normalized (see `recipe_ai.py` / `quantity.py`). The plain `RecipeForm` is the second door; there are two doors in all. |
...
| `DictateButton.jsx` | Speak-to-type control on long-text fields (`lib/speech.js`, browser Web Speech API). The microphone types recognized words as **editable characters into the text field** — `continuous = false`, and **no audio is captured, stored, or played back**. It is a keyboard substitute, not a recording; POSITIONING bans "voice"/"recording"/"in their own words" for exactly this reason. |
...
| `PlantRecipe.jsx` | `/add` | Add-a-recipe flow with **two doors**: "fill it in yourself" (the plain `<RecipeForm>`) and "paste the whole thing" (`<PasteRecipe>`, paste/dictate → LLM-structured draft). Both land on the same `RecipeForm`, which carries the optional "Passed down from" field and `<VisibilityChoice>` (only a dish name is required) → `<SaveCelebration>` (the timer→poof→card reveal, which is now the terminal "saved" screen: tap the card to view, or the button to hand it off) → HandoffInvite. |
...
- **Verifying changes:** backend has `pytest` (**200 tests** across `tests/`);
  frontend has Vitest + React Testing Library (**471 tests in 33 files**) — run
