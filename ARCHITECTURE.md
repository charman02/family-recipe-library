...
| `api/sharing.js` | Recipe + sharing endpoint calls (`plantRecipe`, `deleteRecipe`, `cookRecipe`, `handoffRecipe`, `setVisibility`, `parseRecipeWithAI` — the `POST /recipes/parse` LLM call, `getSharedWithMe`, `getInvitePreview`, `claimInvite`) built on `client`. Was `api/lineage.js`; the tree it was named for is gone. |
...
| `api/posts.js` | The presence-feed calls (`createPost`, `getFeed`, `getPost`, `deletePost`, `getUserPosts` — the `/posts` routes), built on `client`. Powers `Feed`, `PostComposer`, and the profile post grid. |
...
| `PasteRecipe.jsx` | The **primary** add-a-recipe door, a centered **"Say it / Type it" toggle** (speak-first where the browser supports dictation; type-only fallback otherwise). Say-mode is a big mic that transcribes into an editable transcript; Type-mode is one big text box for pasting. Either way the whole recipe goes as free text to `POST /recipes/parse` (LLM) and returns as a pre-filled `RecipeForm` draft to correct before saving. On no key / any failure the endpoint answers `ai: false` and this falls back to the local line-based parser (`lib/parseRecipeText.js`), so the door always works. Amounts are re-graded server-side, never normalized (see `recipe_ai.py` / `quantity.py`). The plain `RecipeForm` is the second door; there are two doors in all. |
...
| `VisibilityControl.jsx` | Owner-only private/public toggle on the recipe page ("Only me" / "Everyone", copy kept in step with `VisibilityChoice`). One surviving italic line names what "Everyone" exposes — the only place a user learns public means "listed in Browse", which round-2 testers were specifically anxious about. (Earlier versions carried dead `parent_recipe_id`/`child_count` lineage branches; those were removed — only explanatory comments remain.) |
...
| `AddChooser.jsx` | `/add` | The add chooser — two doors: **"Share a meal"** → `PostComposer` at `/add/meal`, and **"Keep a recipe"** → `PlantRecipe` at `/add/recipe`. |
| `PostComposer.jsx` | `/add/meal` | Share-a-meal composer: photo + dish name (+ optional line, + optional link to a recipe you own) → `POST /posts`, then the feed. |
| `PlantRecipe.jsx` | `/add/recipe` | Add-a-recipe flow with **two doors**, "say it or paste it" (`<PasteRecipe>`, speak/paste → LLM-structured draft) shown first as the primary card, and "fill it in yourself" (the plain `<RecipeForm>`) below it. Both land on the same `RecipeForm`, which carries the optional "Passed down from" field and `<VisibilityChoice>` (only a dish name is required) → `<SaveCelebration>` (the timer→poof→card reveal, which is now the terminal "saved" screen: tap the card to view, or the button to hand it off) → HandoffInvite. |
...
| `Profile.jsx` | `/profile` | The "You" page — account info, client-side settings (reduce-motion, cooking-mode default), account actions (edit name, change email, change password, delete account), a "Send feedback" button routing to the in-app `/feedback` form, and logout. |
...
- **Verifying changes:** backend has `pytest` (**261 tests** across `tests/`);
  frontend has Vitest + React Testing Library (**474 tests in 39 files**) — run
