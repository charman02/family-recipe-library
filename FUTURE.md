...
1. ~~**Account deletion / anonymization**~~ — **Shipped.** `DELETE /auth/me` performs a hard delete with `CASCADE` (not soft-delete like recipe deletion) and the Profile page exposes a working delete flow. What remains open: deciding what happens to grants already issued and to a recipe sitting on a public Browse feed when the owner deletes their account — those edge cases are worth revisiting but the core privacy gap is closed.
