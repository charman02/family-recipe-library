...
**Stack at a glance:** React + Vite + Tailwind SPA (Vercel) → FastAPI + SQLAlchemy REST API (AWS ECS Fargate) → PostgreSQL (Neon). JWT auth, 54 endpoints, 14 data models, 1043 automated tests (395 pytest + 648 Vitest).
...
**Vitest + React Testing Library** - frontend unit/component tests (648 in 47 files: quantity parsing, imprecise-measure labelling, handoff/invite flows, form and page components, plus design-token invariants). Run with `npm test` in `frontend/`.
