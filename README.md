...
**Stack at a glance:** React + Vite + Tailwind SPA (Vercel) → FastAPI + SQLAlchemy REST API (AWS ECS Fargate) → PostgreSQL (Neon). JWT auth, 43 endpoints, 11 data models, 811 automated tests (305 pytest + 506 Vitest).
...
**Vitest + React Testing Library** - frontend unit/component tests (506 in 41 files: quantity parsing, imprecise-measure labelling, handoff/invite flows, form and page components, plus design-token invariants). Run with `npm test` in `frontend/`.
