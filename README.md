...
**Stack at a glance:** React + Vite + Tailwind SPA (Vercel) → FastAPI + SQLAlchemy REST API (AWS ECS Fargate) → PostgreSQL (Neon). JWT auth, 43 endpoints, 11 data models, 830 automated tests (311 pytest + 519 Vitest).
...
**Vitest + React Testing Library** - frontend unit/component tests (519 in 43 files: quantity parsing, imprecise-measure labelling, handoff/invite flows, form and page components, plus design-token invariants). Run with `npm test` in `frontend/`.
